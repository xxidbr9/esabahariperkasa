...
const Slogan = ({ ...props }) => {
  return (
    ...
    >
      One Stop Service and Stop Solution
    </motion.div>
  );
};
...
const ListServices = ({ ...props }) => {
  return (
    <section className="laptop:py-10 mx-auto grid laptop:grid-cols-2 mobile:grid-cols-1 w-full laptop:gap-x-20 laptop:container mobile:px-4 laptop:px-0 mobile:gap-y-8">
      <motion.div
        initial={{
          y: 100,
          opacity: 0,
      ...
      >
        {listServices
          .filter((service, index) => index === 0 || index % 2 === 0)
          .map((service) => (
            <CardService
              key={service.id}
              image={service.image}
      ...
      >
        {listServices
          .filter((service, index) => index === 1 || index % 2 !== 0)
          .map((service) => (
            <CardService
              key={service.id}
              image={service.image}
...
const CardService = ({ title, text, image, ...props }) => {
  ...
  return (
    <motion.div className="relative flex flex-col gap-y-4 overflow-hidden">
      <motion.div
